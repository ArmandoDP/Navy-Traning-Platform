...
export default function DrawerEditarCliente({ isOpen, cliente, onClose, onSuccess }: Props) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const nombreCompleto = [form.nombre, form.primer_apellido, form.segundo_apellido]
        .filter(Boolean)
        .join(' ')
      ...
      if (membresia) {
        const { error: errMemb } = await supabase.from('membresias').update({
      ...
      alert(`Error al actualizar el cliente: ${err.message || 'Inténtelo de nuevo'}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleResetPassword = async () => {
    if (!cliente.supabase_user_id) return
    try {
  ...
  const handleReenviarCorreo = async () => {
    try {
      const res = await fetch('/api/correo/acceso-cliente', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: cliente.email,
