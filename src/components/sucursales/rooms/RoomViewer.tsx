...
interface Spot {
  ...
  columna:   number
  bloqueado: boolean
  estatus?:  string  // 'Libre' | 'Ocupado' | 'No-show' | 'Bloqueado'
}

interface CeldaLayout {
  fila:    number
  columna: number
  tipo:    string
  numero:  string | null
...
interface Props {
  ...
  alto:      number
  layout:    CeldaLayout[]
  spots?:    Spot[]          // spots con estatus real (para la app)
  readonly?: boolean         // true = solo ver, false = puede seleccionar
...
const ESTATUS_CONFIG: Record<string, { bg: string; border: string; color: string }> = {
  Libre:     { bg: '#e0e7ff', border: '#6366f1', color: '#4338ca' },
  Ocupado:   { bg: '#dcfce7', border: '#22c55e', color: '#15803d' },
  'No-show': { bg: '#fee2e2', border: '#ef4444', color: '#b91c1c' },
  Bloqueado: { bg: '#f3f4f6', border: '#d1d5db', color: '#9ca3af' },
}

export default function RoomViewer({
  nombre, ancho, alto, layout, spots = [], readonly = true, onSpotClick
}: Props) {
  ...
  const cellSize = Math.max(44, Math.floor(300 / ancho))

  const getCelda = (fila: number, columna: number) =>
    layout.find(c => c.fila === fila && c.columna === columna)

  const getSpot = (fila: number, columna: number) =>
    spots.find(s => s.fila === fila && s.columna === columna)

  const renderCeldaContent = (celda: CeldaLayout, spot?: Spot, size?: number) => {
    const s = size || 44
  ...
  if (celda.tipo === 'Rack') return (
    <div className="flex flex-col items-center justify-center w-full h-full gap-0.5 p-1">
      <svg width={cellSize * 0.7} height={cellSize * 0.3} viewBox="0 0 40 12" fill="none">
  ...
  const esReservable = (tipo: string) => tipo === 'Rack' || tipo === 'Mat'
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="text-center">
        <p className="text-base font-black text-gray-900">{nombre}</p>
      </div>

      {/* Grid */}
      <div
        className="inline-grid gap-1.5 p-3 bg-gray-50 rounded-2xl border border-gray-100"
      ...
      >
        {Array.from({ length: alto }, (_, fila) =>
          Array.from({ length: ancho }, (_, columna) => {
            ...
            const spot   = getSpot(fila, columna)
            const tieneItem = !!celda?.tipo
            const reservable = celda && esReservable(celda.tipo)
            const bloqueado  = spot?.bloqueado || spot?.estatus === 'Bloqueado'

            return (
              <div
                key={`${fila}-${columna}`}
                onClick={() => {
                  if (!readonly && reservable && !bloqueado && spot && onSpotClick) {
                    onSpotClick(spot)
                  }
                }}
                className={`rounded-lg border-2 flex items-center justify-center transition-all ${
                  reservable && !readonly && !bloqueado
                    ? 'cursor-pointer hover:scale-105 hover:shadow-md'
                    : tieneItem ? 'cursor-default' : 'cursor-default'
                }`}
                style={{
                  width:  cellSize,
                  height: cellSize,
                  backgroundColor: !tieneItem
                    ? 'transparent'
                    : bloqueado
                      ? ESTATUS_CONFIG['Bloqueado'].bg
                      : spot?.estatus
                        ? ESTATUS_CONFIG[spot.estatus]?.bg || ITEM_CONFIG[celda!.tipo]?.bg
                        : ITEM_CONFIG[celda!.tipo]?.bg || '#f9fafb',
                  borderColor: !tieneItem
                    ? 'transparent'
                    : bloqueado
                      ? ESTATUS_CONFIG['Bloqueado'].border
                      : spot?.estatus
                        ? ESTATUS_CONFIG[spot.estatus]?.border || ITEM_CONFIG[celda!.tipo]?.border
                        : ITEM_CONFIG[celda!.tipo]?.border || '#e5e7eb',
                  opacity: bloqueado ? 0.5 : 1,
                }}
              >
                {tieneItem && renderCeldaContent(celda!, spot, cellSize)}
              </div>
            )
          })
        )}
      ...
      <div className="flex items-center gap-4 flex-wrap justify-center">
        {Object.entries(ESTATUS_CONFIG).map(([estatus, cfg]) => (
          <span key={estatus} className="flex items-center gap-1.5 text-xs text-gray-600">
